...
int main(int argc, const char* argv[]) {
    if ((argc < 2) || (argc > 3) || (argc == 3 && argv[2] && strcmp(argv[2], "-d") != 0)) {
        std::cerr << "Usage: " << argv[0] << " <model.onnx>" << "[optional -d]" << '\n';
        return 0;
    }

    if (!Logger::getInstance().setLogFile(std::string(Config::LOGS_DIR)+std::string(Config::LOGFILE))) {
    ...
    std::string dot_filename = Config::IMG_DIR + replace_extension(argv[1], Config::DOT_EXT);
    std::string png_filename = Config::IMG_DIR + replace_extension(argv[1], Config::PNG_EXT);
    ...
    if (ret != 0) {
        LOG(WARNING, "Failed to render PNG (dot returned " + std::to_string(ret) + ").\n");
    } else {
